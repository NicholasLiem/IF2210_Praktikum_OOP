...
BigNumber BigNumber::operator+(const BigNumber& other) {
  ...
  for (int i = 0; i < BigNumber::getMaxDigit(); i++) {
    c.digit[i] = (this->digit[i] + other.digit[i] + carry) % 10;
    carry = (this->digit[i] + other.digit[i] + carry) / 10;
  }
  return c;
}
...
BigNumber BigNumber::operator-(const BigNumber& other) {
  ...
  for (int i = 0; i < BigNumber::getMaxDigit(); i++) {
    if (this->digit[i] < other.digit[i]){
      c.digit[i] = (this->digit[i] + 10 - other.digit[i]);
      carry = 1;
    } else {
      c.digit[i] = (this->digit[i] - other.digit[i] - carry);
      carry = 0;
    }
  }
  return c;
