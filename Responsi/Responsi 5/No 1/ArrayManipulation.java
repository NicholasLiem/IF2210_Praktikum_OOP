...
class ArrayManipulation {
    ...
    public static <T extends Comparable<T>> T[] mergeAndSortArrays(T[] arr1, T[] arr2) {
        ...
        int panjangtotal = length1 + length2;
        T[] hasil = Arrays.copyOf(arr2, panjangtotal);
        System.arraycopy(arr2, 0, hasil, length1, length2);
        Arrays.sort(hasil);
        return hasil;
    }
    ...
    public static <T> T[] mapArray(T[] arr, Function<T, T> mapper) {
        T[] hasil = Arrays.copyOf(arr, arr.length);
        for (int i = 0; i < arr.length; i++) {
            hasil[i] = mapper.apply(arr[i]);
        }
        return hasil;   
    }
    ...
    public static <T> T[] filterArray(T[] arr, Predicate<T> predicate) {
        ...
        T[] hasil = Arrays.copyOf(arr, count);
        int j = 0;
        for (int i = 0; i < arr.length; i++) {
            if (predicate.test(arr[i])) {
                hasil[j] = arr[i];
